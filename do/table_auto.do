...
eststo: reghdfe lrev scandal other, absorb(id year) vce(robust)
estadd local firm "Yes"
estadd local year "Yes"

eststo: reghdfe lrev envph other, absorb(id year) vce(robust)
estadd local firm "Yes"
estadd local year "Yes"
...
//------------------------   TABLE    ------------------------
********************************************************************************
esttab using "$Tables//Auto.tex", replace booktabs nonotes ///
	...
	addnote("Notes: Robust standard errors in parentheses" "Balanced Sample (2010-2018)" "\sym{*} \(p<0.10\), \sym{**} \(p<0.05\), \sym{***} \(p<0.01\)")
