...
eststo: reghdfe lrev c.envph##c.cfrag1 esg other [aw=nof], absorb(id i.naics5#i.year) vce(robust)
estadd local firm "Yes"
estadd local indyear "Yes"

eststo: reghdfe lrev c.envph##c.cfrag2 esg other [aw=nof], absorb(id i.naics5#i.year) vce(robust)
estadd local firm "Yes"
estadd local indyear "Yes"
...
esttab using "$Results//heterogeneity_w.tex", replace booktabs ///
	drop(_cons cfrag*) label b(3) se(3) ///
	star(* 0.10 ** 0.05 *** 0.01) ///
	title(Heterogeneity - Environmental Fragility (weighted)) ///
	nomtitles ///
